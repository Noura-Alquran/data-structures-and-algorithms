...
function transformToLis(obj){
  // Solution code here...
  const array=[];
  let listForm = Object.entries(obj).map(arr=>{
    array.push(`<li>${arr[0]}: ${arr[1]}</li>`);
  })
return array;
}
...
const addValues = (arr) => {
  // Solution code here...
  let addition=arr.reduce((accm,item)=>{
    accm+=item;
    return accm;
  },0)
  return addition;
...
const reversedString = (str) => {
  // Solution code here...
  const arr=str.split("");
  let reversedStr=arr.reduce((a,b,idx)=>{
