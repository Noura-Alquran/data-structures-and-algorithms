...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  let listItems= recipe.ingredients; 
  listItems.forEach(element => {
  let slicefirst=element.slice(element.indexOf(" ")+1);
  let sliceSecond=slicefirst.slice(slicefirst.indexOf(" ")+1);
   result.push(sliceSecond);
  });
  return result;
};
