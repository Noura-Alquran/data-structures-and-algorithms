...
const toLastNames = people => {
  // Solution code here...
  const array=people.map(person=>person.firstName+' '+person.lastName);
  return array;
...
const validateEmail = (email) => {
  // Solution code here...
  let validEmail = /^\w+\.?\w+@\w+([-.]\w+)*(.net|.com|.org)\b/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let validPhone= /^((\([0-9]{3}\))|[0-9]{3})(\s|\-)?[0-9]{3}(\s|\-)?[0-9]{4}$/;
